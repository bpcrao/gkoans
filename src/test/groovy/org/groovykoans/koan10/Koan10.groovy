...
class Koan10 extends GroovyTestCase {
	...
	void test01_XmlSlurpersReader() {
		...
		def movieRecords= new XmlSlurper();
		def allrecords=movieRecords.parse(new File('F:/gkoans/src/test/groovy/org/groovykoans/koan10/movies.xml'))
		movieCount=allrecords.movie.size()
		...
		// ------------ START EDITING HERE ----------------------
		allrecords.movie.each{
			!(it.title.text().contains('the')||it.title.text().contains('The'))?:
					moviesWithThe.add(it.title.text())
		}
		// ------------ STOP EDITING HERE  ----------------------
		assert moviesWithThe.containsAll([
			'Conan the Barbarian',
			'The Expendables',
			'The Terminator'
		])
		...
		// ------------ START EDITING HERE ----------------------
		def gratThan5= allrecords.movie.findAll{it.@id.toString().toInteger()>5}
		movieIdsGreaterThan5=gratThan5.size()
		// ------------ STOP EDITING HERE  ----------------------
	...
	void test02_XmlSlurpersReader2() {
		...
		List<String> sortedList = allrecords.movie.list().sort{ a, b ->
			if(a.year.toInteger() <=> b.year.toInteger() ==0){
				a.title.toString() <=> b.title.toString()
			}else{
				a.year.toInteger() <=> b.year.toInteger()
			}
		}.title
		// ------------ START EDITING HERE ----------------------
		//sortedList=
