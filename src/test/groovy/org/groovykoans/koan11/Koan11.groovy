...
class Koan11 extends GroovyTestCase {
	...
	void test02_MoreOnInsertingData() {
		...
		Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
			...
			// ------------ START EDITING HERE ----------------------
			def castFile = new File('src/test/groovy/org/groovykoans/koan11/cast.txt')
			castFile.eachLine { line->				
			...
			// ------------ START EDITING HERE ----------------------
			def reader= new XmlSlurper();
			def allrecords=reader.parse(new File('F:/gkoans/src/test/groovy/org/groovykoans/koan11/cast2.xml'))
			def charss=allrecords.character
			charss.each {
				
				db.execute('insert into PERSON (FIRSTNAME, LASTNAME) values (?, ?)', [it.@name.toString(), it.@actor.toString()])
			}
			...
			// ------------ START EDITING HERE ----------------------
			person.add(firstname: 'HHK	', lastname: 'SS')
