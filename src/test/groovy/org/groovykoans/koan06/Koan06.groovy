...
class Koan06 extends GroovyTestCase {
	...
	void test02_CollectMethodOnLists() {
		...
		def uniqueTypes = []
		// ------------ START EDITING HERE ----------------------
		def uniqueTypes2= differentTypes.collect (new TreeSet()){
			it.class.getSimpleName()
		}
		uniqueTypes.addAll(uniqueTypes2)
		// ------------ STOP EDITING HERE  ----------------------
	...
	void test04_ConcludingExercise() {
		...
		range.each {
			...
			def isPrime=true
			def numToCHeck=it
			def range2=2..numToCHeck
			
			range2.each {
				num->
				if(numToCHeck!=num && numToCHeck%num==0){
					System.out.println("Number to check "+numToCHeck+" is nort prime div by num "+num);					
			...
			if(isPrime){				
				primesBetween200And250.add(it)
			}
		}
			// ------------ STOP EDITING HERE  ----------------------
			assert primesBetween200And250 == [
				211,
				223,
				227,
				229,
